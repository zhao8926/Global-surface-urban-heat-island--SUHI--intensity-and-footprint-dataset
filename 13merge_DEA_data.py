...
def merge_global_standard_extent():
    base_folder = r"E:\gis_data\SUHII_DEA_or_SUE\merged_data"
    output_folder = r"E:\gis_data\SUHII_DEA_or_SUE\seasonal_data"
    ...
    for year in range(2018, 2021, 5):

        print(f"Processing year: {year}...")

        for time in times:
            s_path = os.path.join(base_folder, f"SUHII_DEA_{year}_North_{time}.tif")
            w_path = os.path.join(base_folder, f"SUHII_DEA_{year}_South_{time}.tif")

            if not os.path.exists(s_path) or not os.path.exists(w_path):
                print(f"Skip {year}: File missing")
                continue

            out_s_path = os.path.join(output_folder, f"SUHII_DEA_{year}_{time}.tif")
            out_w_path = os.path.join(output_folder, f"SUHII_DEA_{year}_{mapping[time]}.tif")
