...
def merge_global_standard_extent():
    base_folder = r"E:\gis_data\SUHII_Aqua_HTIA"
    output_summer_folder = r"E:\gis_data\SUHII_Aqua_HTIA_Summer"
    output_winter_folder = r"E:\gis_data\SUHII_Aqua_HTIA_Winter"
    ...
    for year in range(2005, 2025):
        ...
        s_path = os.path.join(base_folder, f"SUHII_global_{year}_summer_{time_phase}.tif")
        w_path = os.path.join(base_folder, f"SUHII_global_{year}_winter_{time_phase}.tif")

        if not os.path.exists(s_path) or not os.path.exists(w_path):
            print(f"Skip {year}: File missing")
            continue

        out_s_path = os.path.join(output_summer_folder, f"HTIA_summer_{time_phase}_{year}.tif")
        out_w_path = os.path.join(output_winter_folder, f"HTIA_winter_{time_phase}_{year}.tif")
        ...
        print(f"{year} Processing complete. Output size: {global_width}x{global_height}")
