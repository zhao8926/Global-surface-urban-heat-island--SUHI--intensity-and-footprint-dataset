...
out_xlsx = fr'E:\data_analysis\UHI\statistics20260110\FRAC_statistics_ISA.xlsx'
df = pd.read_excel(xlsx)

target_cols = df.filter(regex=fr'^ISA\d+$').columns

stats_df = df[target_cols].agg(['mean', 'std', 'min', 'max']).transpose()

stats_df['Year'] = stats_df.index.str.extract(fr'ISA(\d+)', expand=False)
